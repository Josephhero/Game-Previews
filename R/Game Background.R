...
background_function <- function(f_sched = sched, 
  ...
  game_data <- f_sched |> 
    ...
    mutate(gametime_label = sub("^0", "", format(as.POSIXct(paste0(gameday, " ", gametime)), "%I:%M %p"), '%r'), 
           .after = gametime)
  ...
  (
    p <- ggplot(df, aes(x = x, y = y)) +
      ...
      ggimage::geom_image(data = data.frame(
        ...
        aes(x = x, y = y, image = image), 
        size = 0.16) +
      shadowtext::geom_shadowtext(data = data.frame(x = 5, y = 8.5), 
                                  aes(x = x, y = y, label = "@"), 
                                  color = "white", 
                                  bg.color = "black",
                                  size = 18
      ...
      geom_label(data = 
                   data.frame(
                     x = 5, 
                     y = 9.6), 
                 aes(label = paste0(game_data$weekday, ", Week ", f_week, ", ", game_data$gametime_label, " EST")), 
                 size = 12, 
                 color = "#daa520", 
                 fill = "gray10",
                 fontface = "bold", 
